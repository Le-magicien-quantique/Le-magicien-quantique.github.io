...
function createChallengeList(props: ChallengeListProps): Challenge[] {
  const { excludeNum, solution, max_challenges, craft_url, locale } = props;
  ...
  for (let i = 1; i <= max_challenges; i++) {
    if (i !== excludeNum) {
      const tmp_url = craft_url(false, i.toString(), false);
      challenges.push({
        label: `Challenge ${i}`,
        link: "/" + locale + tmp_url,
      });
  ...
  const tmp_url = craft_url(false, excludeNum.toString(), !solution);
  challenges.push({
